...
const uploadFile = async (file, uploadDir) => {
  if (file && file.length > 0) {
    ...
    const imageFilename = `${uniqueId}_${file[0].originalFilename}`;
    const newPath = path.join(uploadDir, imageFilename);

    const allowedTypes = ["image/jpeg", "image/png"];
    const maxSize = 5 * 1024 * 1024; // 5MB
    if (!allowedTypes.includes(file[0].mimetype) || file[0].size > maxSize) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const logoFileName = await uploadFile(files.logo, uploadDir);
    const paymentProofFileName = await uploadFile(files.paymentProof, uploadDir);
    ...
    const billingData = {
      restaurant_id: newUser._id,
      plan: "Standard",
      planAmount: 10000,
      paymentMethod: paymentMethod[0],
      cardholder_name: cardholderName[0],
      ...
      <div style="background-color: #f0f4f8; padding: 40px; font-family: Arial, sans-serif;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
          ...
          <div style="padding: 30px;">
            ...
            <p style="font-size: 16px; color: #333333; line-height: 1.6;">
              Your request has been sent to the super admin for approval. You will receive your credentials once your account is verified.
            </p>
            <p style="font-size: 16px; color: #333333; line-height: 1.6;">
    ...
    await sendVerificationEmail(email[0], htmlContent);

    return res.status(200).json({ message: "Verification email sent successfully. Your request has been sent to the super admin for approval. You will receive your credentials once your account is verified." });
  } catch (error) {
